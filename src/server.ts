...
import { 
    sendInitialOptions,
    sendOptionsInsurance,
    sendOptionsIntermedInsurance,
  removeCommand, 
  sendMessage, 
  sendImage, 
  toTitleCase, 
...
app.post("/hook", async (req:any, res:any) => {
    ...
    if (value?.messages) {
        ...
        /*if(message){
            try {
                sendMessage(destination, `
                    No momento eu não garanto que responderei imediatamento,por isso criei essas opções para facilitar seu atendimento.
                    Digite o número da opção desejada:
                    1 - Saber qual o telefone da assistencia 24 horas.
                    2 - Solicitar um orçamento.
                    3 - Deixar um recado para eu retornar assim que possivel.
                    5 - Encerrar.
                `, id);
            } catch (error) {
                sendMessage(destination, `
        ...
        if (message) {
            ...
            if(message.contains(".")){                
                let {cpf,cnpj} = await searchUser(message) 
                if(cpf){
                    cpf = `Placa: ${cpf.Placa},
                Seguradora: ${cpf.Seguradora},
                Modalidade: ${cpf.CPF},
                Telefone Capitais: ${cpf.CPF},
                Telefone outras localidades: ${cpf.CPF},
                "Link WhatsApp": ${cpf.CPF}`
                }
                
                try {
                    sendMessage(destination,cpf,id)
                } catch (error) {
                    sendMessage(destination, `${error}`, id);
                }
            }


            try {
                sendInitialOptions(destination,id)
                //sendNewChat(destination,id)
            } catch (error) {
                sendMessage(destination, `${error}`, id);
            }
        ...
        if (interactive) {
            ...
            if (buttonReply) {
                switch(buttonReply.id) {
                    case "sim":
                        sendOptionsInsurance(destination,id)
                        break
                    case "seguradora":
                        sendOptionsIntermedInsurance(destination,id)
                        
                        break
                    case "não.sabe.seguradora":
